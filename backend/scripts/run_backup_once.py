...
from backend.app.backup_service import backup_rule  # noqa: E402
...
async def main(peer_id: int | None) -> None:
    ...
    async with SessionLocal() as db:
        ...
        print(f"开始备份 rule={rule.id} peer={rule.peer_id} title={rule.chat_title}")
        rule_id = rule.id
    try:
        run_id = await backup_rule(rule_id, trigger="cli")
    finally:
        await runtime_manager.stop()
        await engine.dispose()
    print(f"备份完成 run={run_id}")
